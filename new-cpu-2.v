...
module MainControl (Op,Control); 
  input [3:0] Op;
  output reg [8:0] Control; //Control is now 11 bits.
...
// Test Program
  initial begin 
   //Modified Instructions that showcase full instruction set architecture is accomplished here.
    ...
    DMemory[1] = 16'd5; // swap the cells and see how the simulation output changes
    DMemory[0] = 16'd7;
     // Test 3: beq TAKEN
  ...
  assign MemReturn = DMemory[ALUOut >> 1]; //Handles the memory adress, very similar to top one.
  ...
  //New mux needed to account for data memory.
  muxB write_back(ALUOut, MemReturn, MemtoReg, WD);
  //ALUOut repaced with WD since write back must be handled for both instruction and data memory.
  ...
  always @(negedge clock) begin 
    PC <= PCNext; //Next pc adress used on falling edge.
    if (MemWrite) //If the memory write signal is active then this will run.
       DMemory[ALUOut>>1]<=RD2; //The next data memory adress is found using the value in ALUOut.
  end
endmodule
