...
module reg_file (RR1,RR2,WR,WD,RegWrite,RD1,RD2,clock);
  ...
  input RegWrite,clock; //Clock pulses determine execution, Reg Write determines whether to write to the register file.
  ...
  always @(negedge clock) //At the falling edge of the clock, this will run.
    if (RegWrite==1 & WR!=0) //Prevents $zero from being overwritten and make sure only when asserted that Write Register is allowed.
    Regs[WR] <= WD; //The destination register is assigned the data using the appropriate register number.
endmodule
